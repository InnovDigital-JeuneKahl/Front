// pages/index.tsx
"use client";

import React from "react";
import Link from "next/link";
import { Swiper, SwiperSlide } from "swiper/react";
import { Navigation, Pagination, A11y } from "swiper/modules";
import Marquee from "react-fast-marquee";

import "swiper/css";
import "swiper/css/navigation";
import "swiper/css/pagination";

// ---------- Data ----------
const navLinks = [
  { label: "Home", href: "#" },
  { label: "Company", href: "#" },
  { label: "Product", href: "#" },
  { label: "Pricing", href: "#" },
];

const heroContent = {
  title: "Clarity in your data.",
  subtitle: "Power in your decisions",
  description:
    "At Freightmate, we believe that organization is the cornerstone of optimization. Our PMS and TMS are meticulously designed to streamline operations to move swiftly and efficiently in the complex world of logistics.",
  cta: {
    text: "Press",
    key: "⏎",
    suffix: "any time to get Started",
  },
};

const dashboardPreview = {
  src: "https://placehold.co/900x400?text=Dashboard+Preview",
  alt: "Dashboard Preview",
};

const features = [
  {
    img: {
      src: "https://placehold.co/320x220?text=Analytics+Chart",
      alt: "Analytics Chart",
      className: "rounded-xl shadow border-2 border-[#e9d8fd] bg-[#f5f3ff] p-2",
    },
    title: "Benefit from the best technologies",
    titleClass: "text-indigo-800",
    description:
      "Freightmee revolutionizes logistics with seamless carrier collaboration, eliminating IT hassles. Swiftly allocate trucks and drivers with feedback for efficient operations.",
    reverse: false,
  },
  {
    img: {
      src: "https://placehold.co/320x220?text=Shipments+Card",
      alt: "Shipments Card",
      className: "rounded-xl shadow border-2 border-[#fbcfe8] bg-[#fdf2f8] p-2",
    },
    title: "The new way to drive your loading",
    titleClass: "text-pink-600 text-right md:text-left",
    description:
      "Total control of all your loads, to save you time, focus your efforts on the essentials and offer your customers an optimal quality of service.",
    reverse: true,
  },
  {
    img: {
      src: "https://placehold.co/320x220?text=Working+Times",
      alt: "Working Times",
      className: "rounded-xl shadow border-2 border-[#e9d8fd] bg-[#f5f3ff] p-2",
    },
    title: "Set up and manage an agile transport",
    titleClass: "text-indigo-800",
    description:
      "Freightmee offers a versatile transportation plan, managing daily and weekly volumes with ease. Empower your teams with dedicated support, and efficiently monitor quality to satisfy carriers and ensure optimized logistics.",
    reverse: false,
  },
];

const testimonials = [
  {
    logo: "https://placehold.co/120x40?text=logoipsum",
    text: "“FreightMee Is A Logistics Superhero! Their Tracking Is Like Magic, And The Team Is Always Ready With A Friendly High-Five.”",
    userImg: "https://randomuser.me/api/portraits/men/32.jpg",
    userName: "Wadji Boumenjel",
    userTitle: "CEO Of Logoipsum",
  },
  {
    logo: "https://placehold.co/120x40?text=logoipsum2",
    text: "“FreightMee helped us scale our operations with ease. Their support team is fantastic!”",
    userImg: "https://randomuser.me/api/portraits/women/44.jpg",
    userName: "Jane Doe",
    userTitle: "COO Of LogiPro",
  },
  {
    logo: "https://placehold.co/120x40?text=logoipsum3",
    text: "“The analytics dashboard is a game changer for our business.”",
    userImg: "https://randomuser.me/api/portraits/men/45.jpg",
    userName: "John Smith",
    userTitle: "Logistics Manager",
  },
];

const trustedLogos = [
  "https://placehold.co/100x40?text=UPS",
  "https://placehold.co/100x40?text=FedEx",
  "https://placehold.co/100x40?text=DHL",
  "https://placehold.co/100x40?text=Maersk",
  "https://placehold.co/100x40?text=Amazon",
  "https://placehold.co/100x40?text=USPS",
  "https://placehold.co/100x40?text=Logitech",
];

// ---------- Components ----------
const FeatureCard = ({ feature }: any) => {
  return (
    <div
      className={`flex flex-col ${
        feature.reverse ? "md:flex-row-reverse" : "md:flex-row"
      } items-center gap-10 w-full mb-20`}
    >
      <div>
        <img
          src={feature.img.src}
          alt={feature.img.alt}
          className={feature.img.className}
        />
      </div>
      <div className="flex-1">
        <h3 className={`font-bold text-lg mb-2 ${feature.titleClass}`}>
          {feature.title}
        </h3>
        <p className="text-gray-600">{feature.description}</p>
      </div>
    </div>
  );
};

const TestimonialCard = ({ testimonial }: any) => {
  return (
    <div className="flex flex-col items-center p-6">
      <img src={testimonial.logo} alt="Logo" className="mb-4" />
      <p className="text-center text-gray-700 mb-4">{testimonial.text}</p>
      <div className="flex items-center gap-4">
        <img
          src={testimonial.userImg}
          alt={testimonial.userName}
          className="w-12 h-12 rounded-full"
        />
        <div>
          <p className="font-semibold">{testimonial.userName}</p>
          <p className="text-sm text-gray-500">{testimonial.userTitle}</p>
        </div>
      </div>
    </div>
  );
};


const CtaBanner = () => {
    return (
      <section className="w-full bg-gradient-to-r from-zinc-800 to-zinc-900 text-white rounded-2xl p-6 md:p-10 mb-12 shadow-lg max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
        <div>
          <h2 className="text-xl font-semibold">We are at your disposal</h2>
          <div className="flex items-center gap-4 text-sm mt-2 text-gray-300 flex-wrap">
            <span>📞 +213 770 80 80 00</span>
            <span>✉️ contact@freightmee.com</span>
            <span>📍 340 Rue Dar El Heddad, Les Pins Maritimes, Mohammadia, Alger</span>
          </div>
        </div>
        <button className="bg-pink-500 hover:bg-pink-600 text-white font-semibold px-6 py-2 rounded-full transition-all">
          Get Started →
        </button>
      </section>
    );
  };

  const Footer = () => {
    return (
      <footer className="w-full bg-white text-gray-600 pt-16 pb-8 px-6 border-t mt-16">
        <div className="max-w-6xl mx-auto grid grid-cols-2 md:grid-cols-5 gap-8">
          {/* Logo */}
          <div className="col-span-2 md:col-span-1">
            <img src="/logo.svg" alt="FreightMee Logo" className="w-32 mb-4" />
          </div>
  
          {/* Links */}
          <div>
            <h4 className="font-semibold mb-2">Product</h4>
            <ul className="space-y-1 text-sm">
              <li>Features</li>
              <li>Product</li>
              <li>Shipper</li>
              <li>Carrier</li>
              <li>Pricing</li>
            </ul>
          </div>
          <div>
            <h4 className="font-semibold mb-2">Company</h4>
            <ul className="space-y-1 text-sm">
              <li>Our Team</li>
              <li>Careers</li>
              <li>Privacy Policy</li>
              <li>Terms of Service</li>
              <li>Legal Notice</li>
            </ul>
          </div>
          <div>
            <h4 className="font-semibold mb-2">Resources</h4>
            <ul className="space-y-1 text-sm">
              <li>Blog</li>
              <li>Q&A</li>
              <li>Medium</li>
            </ul>
          </div>
  
          {/* Language Selector */}
          <div className="col-span-2 md:col-span-1 flex items-start md:items-end justify-start md:justify-end">
            <select className="border rounded px-3 py-1 text-sm">
              <option value="en">🇬🇧 English</option>
              <option value="fr">🇫🇷 French</option>
            </select>
          </div>
        </div>
  
        {/* Bottom Bar */}
        <div className="max-w-6xl mx-auto mt-10 flex flex-col md:flex-row justify-between items-center text-xs text-gray-400 border-t pt-6 gap-4">
          <div className="flex gap-4">
            <a href="#" className="hover:text-indigo-600">LinkedIn</a>
            <a href="#" className="hover:text-indigo-600">Instagram</a>
          </div>
          <p>© 2024 Freightmee. All rights reserved.</p>
        </div>
      </footer>
    );
  };
  

// ---------- Main Page ----------
const LandingPage = () => {
  return (
    <main className="flex flex-col items-center px-6 md:px-20 max-w-screen-xl mx-auto">
      {/* Navbar */}
      <nav className="my-10">
        <div className="flex gap-8 bg-[#f5f3ff] py-3 px-6 rounded-full shadow-sm">
          {navLinks.map((link) => (
            <Link key={link.label} href={link.href} className="text-indigo-700">
              {link.label}
            </Link>
          ))}
        </div>
      </nav>

      {/* Hero */}
      <section className="text-center my-16 max-w-3xl">
        <h1 className="text-4xl font-bold mb-2">{heroContent.title}</h1>
        <h2 className="text-4xl text-indigo-800 font-bold mb-4">
          {heroContent.subtitle}
        </h2>
        <p className="text-gray-600 mb-6">{heroContent.description}</p>
        <button className="bg-indigo-700 text-white px-6 py-2 rounded-full shadow">
          {heroContent.cta.text} <kbd>{heroContent.cta.key}</kbd> {heroContent.cta.suffix}
        </button>
        <img
          src={dashboardPreview.src}
          alt={dashboardPreview.alt}
          className="mt-10 rounded-xl shadow-lg"
        />
      </section>

      {/* Features */}
      <section className="w-full my-24">
        {features.map((feature, i) => (
          <FeatureCard key={i} feature={feature} />
        ))}
      </section>

      {/* Testimonials */}
      <section className="w-full my-24">
        <h2 className="text-3xl font-bold text-center mb-10">
          We focus on the efficiency of freight networks
        </h2>
        <Swiper
          modules={[Navigation, Pagination, A11y]}
          spaceBetween={50}
          slidesPerView={1}
          navigation
          pagination={{ clickable: true }}
        >
          {testimonials.map((testimonial, i) => (
            <SwiperSlide key={i}>
              <TestimonialCard testimonial={testimonial} />
            </SwiperSlide>
          ))}
        </Swiper>
      </section>

      {/* Trusted by */}
      <section className="w-full my-16">
        <h2 className="text-center text-gray-700 text-lg font-semibold mb-6">
          Trusted by teams at
        </h2>
        <Marquee gradient={false} speed={50}>
          {trustedLogos.map((logo, i) => (
            <img
              key={i}
              src={logo}
              alt={`Logo ${i}`}
              className="mx-10 h-10 grayscale hover:grayscale-0 transition-all duration-300"
            />
          ))}
        </Marquee>
      </section>
      <CtaBanner />
      <Footer />

    </main>
  );
};

export default LandingPage;
